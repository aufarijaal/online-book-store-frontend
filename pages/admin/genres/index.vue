...
async function getData(query?: Filters) {
  ...
  const result = await useApiFetch('/admin/genres', {
    query,
    headers: {
  ...
  data.value = (result.data.value as any).data.data as GenreData[]
...
async function remove(ids: number[]) {
  try {
    ...
    const result = await useApiFetch('/admin/genres?ids=' + ids.join(','), {
      method: 'DELETE',
      headers: {
        Accept: 'application/json',
      },
...
<template>
  <div class="p-4 pt-0">
    <Teleport to="body">
      <ModalsAdminAddEditGenre v-if="showModal" @cancel="() => {
        showModal = false
    ...
    <DataTable title="Genres" :columns="columns" :data="data" :pagination-info="paginationInfo"
