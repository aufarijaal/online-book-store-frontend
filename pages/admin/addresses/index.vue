...
async function editAddress() {
  ...
  const result = await useApiFetch(`/api/v1/admin/addresses/${selectedIds.value[0]}/edit`, {
    headers: {
      Accept: 'application/json',
    },
  })
  ...
  addressToEdit.value.full_address = (result.data.value as any).data.full_address
...
async function deleteAddresss() {
  ...
  if (confirm(message)) {
    ...
    const result = await useApiFetch('/api/v1/admin/addresses?ids=' + selectedIds.value.join(','), {
      method: 'DELETE',
      headers: {
        Accept: 'application/json',
      },
...
<template>
  <div class="p-4 pt-0">
    <Teleport to="body">
      <ModalsAdminAddAddress v-if="showAddAddressModal" @close="showAddAddressModal = false" @refresh="getAddresss" />
    ...
    <div class="w-100">
      <div class="datatable-toolbar d-flex align-items-center mb-3 p-2 gap-2">
        ...
        <div class="input-group">
          ...
          />
          <button class="btn btn-primary" type="button" id="button-search" @click="getAddresss">
      ...
      <div class="card">
        <h5 class="card-header d-flex align-items-center justify-content-between">
          ...
          <div class="d-flex align-items-center gap-2">
            <button class="btn btn-primary btn-sm" @click="showAddAddressModal = true">
              <PlusIcon />
            </button>
            <button class="btn btn-success btn-sm" :disabled="!selectedIds.length" @click="editAddress">
              <PencilIcon />
            </button>
            <button class="btn btn-danger btn-sm" :disabled="!selectedIds.length" @click="deleteAddresss">
              <TrashIcon />
              ({{ selectedIds.length }})
            </button>
            <button
          ...
          <table class="table table-striped" v-if="addressResponse">
                ...
                <th>
                  <div class="form-check">
                    <input
                      ...
                      "
                      :checked="selectedIds.length === addressResponse.data.data.length"
            ...
            <tbody class="table-border-bottom-0">
              <tr v-for="address in addressResponse?.data.data">
                ...
                <td>
                  <div
                    :class="['box-pulse', address.is_active ? 'bg-success pulse' : 'bg-dark']"
