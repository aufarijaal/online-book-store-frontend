...
const submit = handleSubmit(async (values) => {
  ...
  const result = await useApiFetch('/addresses' + (props.existing && props.existing.id ? `/${props.existing.id}` : ''), {
...
<template>
  <ModalsBaseModal :title="existing && existing.id ? 'Edit address' : 'Add new address'" @close="$emit('cancel')">
    ...
    <form class="d-flex flex-column gap-2" id="add-edit-address-form" @submit.prevent="submit">
    ...
    <template #actions>
      <button type="submit" form="add-edit-address-form" class="btn btn-primary btn-sm">Submit</button>
    </template>
  </ModalsBaseModal>
</template>
