...
async function getData() {
    try {
        ...
        const result = await useApiFetch('/admin/genres?forDropdown=true', {
            headers: {
...
<template>
    ...
    <div :class="['d-flex flex-column', wrapperClass]" v-else>
        <div class="input-group">
            ...
            <select :class="{ 'form-select': true, 'border border-danger': error }" aria-describedby="genreHelp"
                v-model="model">
        ...
        <div id="genreHelp" class="form-text text-xs text-danger" v-if="error">
            {{ error }}
        </div>
    </div>
</template>
